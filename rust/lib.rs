...
pub trait Rust {
	...
	fn highlight(&mut self, file_id: FileId) -> Result<Vec<HighlightedRange>>;
	fn diagnostics(&self, file_id: FileId) -> Result<Vec<Diagnostic>>;
...
impl ipc::Server for Box<dyn Rust> {
	...
	type Item = Item;
	fn reply(&mut self, item: Item) -> Result<Vec<u8>> {
		#[throws] fn serialize<T:Serialize>(r : Result<T>) -> Vec<u8> { ipc::serialize(&r.map_err(|e| e.to_string()))? }
		use Item::*;
		match item {
...
pub fn highlight(file_id: FileId) -> Result<Vec<HighlightedRange>> { request::<HighlightFile>(Item::HighlightFile(HighlightFile{file_id})) }
pub fn diagnostics(file_id: FileId) -> Result<Vec<Diagnostic>> { request::<Diagnostics>(Item::Diagnostics(Diagnostics{file_id})) }
